...
for entity in table_client.list_entities(select=["Updated", "Domain"]):
    ...
    # Keep only timestamps inside the month window
...
# -----------------------------
# Compute busiest second per domain
# -----------------------------
busiest = {}

for domain, seconds in freq.items():
    max_second, max_count = max(seconds.items(), key=lambda kv: kv[1])
    busiest[domain] = {
        "timestamp": max_second,
        "count": max_count
    }
...
for domain, info in busiest.items():
    ts_int = info["timestamp"]
    ts_dt = datetime.fromtimestamp(ts_int, tz=timezone.utc)
    logging.info(
        "%s: busiest second = %s with %d creations",
        domain,
        ts_dt.isoformat(),
        info["count"],
    )
